#класс хранящий данные о дереве
#имеет обработку неправильно введенного количества healthy
#динамич. атр., спец(маг. метод) __str__,  свойство, getter, setter

'''
я бы делал логику для 1-4 функций в 14 варианте в отдельных функцию ProgramManager, а в классе бы просто возращал данные.
То есть класс выполняет главную логику, а уже ввод, проверка и вывод делаются в бизнес-функциях ProgramManager

как работает Pickle и csv
'''
...
class BaseManager():
    def __init__(self, filepath: str, records: dict):
        ...
        #используется класс ConvertDict
...
class ProgramManager():
    ...
    #доделать сохранение и загрузку, так же сделать пользовательский ввод filepath и разобраться как будет меняться filepath
    #если создается мэнеджер на основе пути и получается надо каждый раз пересоздавать, или просто функция будет получать путь
    #либо забить и просто сделать функцию получения пути и работать с этим путем и отдельную фукнцию смены пути, звучит неплохо
    #а так задача плюс минус готова